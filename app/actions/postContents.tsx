...
export async function postContents(preeData:contentsActionType, formData:FormData):Promise<contentsActionType>{
  try{
  ...
  if (!user) {
    ...
    const {data, error} = await supabase.from('contents').insert([{title, description, content_type_id:contentTypeId, category_id:categoryId, tech_id:techId, content: actualContent, part, created_by: user.id}]);

    if(error){
      return {success:false, message: error.message}
    }else{
      return {success:true, message: `${contentType} is created successfully!`}
    }
